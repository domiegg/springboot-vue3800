		...
		<view class="container" :style='{"width":"100%","padding":"0","position":"relative","background":"#DFD8CC","height":"auto"}'>
            <view :style='{"padding":"20rpx 30rpx","margin":"-40rpx 0 0 0","overflow":"hidden","borderRadius":"40rpx 40rpx 0 0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto","zIndex":"1111"}' class="detail-content">
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"width":"100%","padding":"0","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0","color":"rgb(255, 255, 255)","background":"rgb(255, 170, 51)","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="userid&&isAuth('discusssutuohuodong','私聊')" @tap="chatClick">联系TA</button>
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0","color":"rgb(255, 255, 255)","background":"rgb(255, 170, 51)","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="!userid&&isAuthFront('discusssutuohuodong','私聊')" @tap="chatClick">联系TA</button>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusssutuohuodongCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusssutuohuodongCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discusssutuohuodong', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','discusssutuohuodong');
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','discusssutuohuodong');
			...
			async init(type=1){
				...
				let res = await this.$api.info('discusssutuohuodong', this.id);
				let reg=new RegExp('http://localhost:8080/springboot36u45959/upload','g')//g代表全部
				this.detail = res.data;

				if(type==2){
					this.detail.discussnum++
					await this.$api.update('discusssutuohuodong',this.detail)
				}
			},
			...
			async upCallback(mescroll) {
				if(uni.getStorageSync("appUserid")){
					let res = await this.$api.list('discussdiscusssutuohuodong', {
						page: mescroll.num,
						limit: 10,
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscusssutuohuodong/add-or-update?refid=${this.id}`)
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscusssutuohuodong',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discusssutuohuodong',that.detail)
							setTimeout(()=>{
