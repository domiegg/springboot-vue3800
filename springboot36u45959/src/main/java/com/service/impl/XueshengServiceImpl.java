...
public class XueshengServiceImpl extends ServiceImpl<XueshengDao, XueshengEntity> implements XueshengService {
	...
	public PageUtils queryPage(Map<String, Object> params, Wrapper<XueshengEntity> wrapper) {
		...
		List<XueshengView> records = baseMapper.selectListView(page, wrapper);
		for (XueshengView record : records) {
			Integer i = baseMapper.selectFenshu(record);
			if (i == null) i = 0;
			record.setJifen(i + record.getJifen());
		}
	...
	@Override
	public Integer selectFenshu(XueshengEntity u) {

		return baseMapper.selectFenshu(u);
	}
